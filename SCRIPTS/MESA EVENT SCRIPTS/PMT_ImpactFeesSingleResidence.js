...
try {
	...
	if (!classification) logDebug("Classification not found. No impact fees assessed");
		...
		if (classification == "Single Family-Detached (per dwelling unit)"){
			...
			// if the fee qty has changed on an invoiced fee, void the fee
			...
			// assess the fee
			if (!feeExists("RDIF160")) addFee("RDIF160", "PMT_RDIF", "FINAL", fireQty, "N");
			if (!feeExists("RDIF210")) addFee("RDIF210", "PMT_RDIF", "FINAL", publicSafetyQty, "N");
			if (!feeExists("RDIF260")) addFee("RDIF260", "PMT_RDIF", "FINAL", stormWaterQty, "N");
			if (!feeExists("RDIF310")) addFee("RDIF310", "PMT_RDIF", "FINAL", resDevQty, "N");
			if (!feeExists("RDIF010")) addFee("RDIF010", "PMT_RDIF", "FINAL", waterQty, "N");
			if (!feeExists("RDIF060")) addFee("RDIF060", "PMT_RDIF", "FINAL", wasteWaterQty, "N");
			...
			// assess the fee
			if (!feeExists("RDIF170")) addFee("RDIF170", "PMT_RDIF", "FINAL", fireQty, "N");
			if (!feeExists("RDIF220")) addFee("RDIF220", "PMT_RDIF", "FINAL", publicSafetyQty, "N");
			if (!feeExists("RDIF270")) addFee("RDIF270", "PMT_RDIF", "FINAL", stormWaterQty, "N");
			if (!feeExists("RDIF320")) addFee("RDIF320", "PMT_RDIF", "FINAL", resDevQty, "N");
			if (!feeExists("RDIF020")) addFee("RDIF020", "PMT_RDIF", "FINAL", waterQty, "N");
			if (!feeExists("RDIF070")) addFee("RDIF070", "PMT_RDIF", "FINAL", wasteWaterQty, "N");
		}
	}
} catch (err){
